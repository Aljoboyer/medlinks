...
export default function BottomCoupleButton({customStyle, nextHandler, backHandler, isLoad, BackBtn = true, title1= 'Back', title2='Next'}) {
  return (
    <View style={[styles.btnView, customStyle]}>
        {
            BackBtn && <Pressable onPress={backHandler} style={styles.backBtn}>
            <Text style={styles.backBtnText}>{title1}</Text>
        </Pressable>
        }
       
        {
            ...
            <Text style={styles.nextBtnText}>{title2}</Text>
        </Pressable>
        }
