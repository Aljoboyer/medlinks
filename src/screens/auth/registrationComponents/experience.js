...
import { getExperienceList } from '../../../Redux_Mine/Redux_Slices/CompleteProfileTabSlice';
...
export default function Experience({ page, setPage }) {
          ...
          updateExperience(
            expID: "${ExperienceList[0].expID}",
              ...
              skillIDs: "${finalSelectedSkill?.length > 0 ? SkilIdArr : ""}",
                ...
                designationID: ${Number(jobRole?.hciID)},
                jobType: "",
                startingMonth: ${Number(joiningMonth) ? Number(joiningMonth) : ExperienceList[0]?.startingMonth ? ExperienceList[0]?.startingMonth : 0},
                startingYear: ${Number(joiningYear) ? Number(joiningYear) : ExperienceList[0]?.startingYear ? ExperienceList[0]?.startingYear : 0},
                workingMonth: 0,
                workingYear: 0,
                currentlyWorking: ${currentlyWorking ? Boolean(Number(1)) : Boolean(Number(0))},
                noticePeriodID: ${Number(noticePeriod?.npID) ? Number(noticePeriod?.npID) : 0},
  ...
  const experienceHandler = () => {
    ...
    if (isExperience == 'Yes') {
      ...
      if (!isNoticePeriod || (isNoticePeriod == 'Yes' && !noticePeriod?.npID)) {
      ...
      if (!isExperience) {
        ...
        const QUERY_UPDATE_EXPSALARY = {
          query: `mutation MyMutation {
                  updateExpSalary(
                    exp: ${totalExp == 6 ? Number(totalExp) : 0}, 
                    expMonths: ${totalExp !== 6 ? Number(totalExp) : 0},
                    salary: ${monthlySalary ? Number(monthlySalary) : 0},
                    workStatus: "${isExperience == 'Yes' ? 'Experienced' : 'Fresher'}"
                  )
                }
                `,
        ...
        gqlquery(QUERY_UPDATE_EXPSALARY, null)
          ...
          .then((data) => {
            setLoading(false)
            setPage(3)
            console.log('QUERY_UPDATE_EXPSALARY ', data)
          })
      }
    }
  }
  ...
  useEffect(() => {
    getEditSkills()
    if (ExperienceList[0]?.startingYear) {
      const dates = ExperienceList[0]?.startingMonth + '/' + ExperienceList[0]?.startingYear
      setJoiningDate(dates)
      setJoiningMonth(ExperienceList[0]?.startingMonth)
      setJoiningYear(ExperienceList[0]?.startingYear)
    }
    if (ExperienceList[0]?.expID) {
      getEditDepartMent()
      setTotalExp({label: `${totalExpData?.exp ? `${totalExpData?.exp} Years` : `${totalExpData?.expMonths} Months`}`, value: totalExpData?.exp || totalExpData?.expMonths})
      setMonthlySalary(monthlySalaryData)
      const findNotice = allNoticePeriodData?.find((item) => item?.npID == ExperienceList[0]?.noticePeriodID)
      if (findNotice?.npID) {
    ...
    if (ExperienceList[0]?.designationID) {

      if (ExperienceList[0]?.isOtherSkill) {
        setJobRole({ specialty: ExperienceList[0]?.designation, hciID: 0 })

      } else {
        setJobRole({ specialty: ExperienceList[0]?.designation, hciID: ExperienceList[0]?.designationID })
      }
    }

    if (ExperienceList[0]?.healthInstituteID) {
      setIsExperience('Yes')
      if (ExperienceList[0]?.isOtherInstitute == false) {
        setInstitutionName({ name: ExperienceList[0]?.instituteName, himID: ExperienceList[0]?.healthInstituteID })
      } else {
        setInstitutionName({ name: ExperienceList[0]?.instituteName, himID: 0 })

      }
    }

    if (ExperienceList[0]?.isOtherIndustry) {
      console.log('other e dukse')
      setIndustry({ industry: 'Other', hciID: ExperienceList[0]?.healthInstituteTypeID })
      setIndustryOther(ExperienceList[0]?.instituteType)
    }

    if (!ExperienceList[0]?.isOtherIndustry) {
      console.log('oeanother')
      setIndustry({ industry: ExperienceList[0]?.instituteType, hciID: ExperienceList[0]?.healthInstituteTypeID })
    }

    if (!ExperienceList[0]?.expID) {
      setIsExperience('No')
    }
  }, [ExperienceList[0]?.expID, isFocused, page])

  const getEditDepartMent = async () => {
    const QUERY_GET_DEPARTMENT_BY_ID = {
      query: `query MyQuery {
              getDepartmentByID(departmentID: ${ExperienceList[0]?.departmentID}) {
                departmentID
                name
              }
            }`
    ...
    gqlquery(QUERY_GET_DEPARTMENT_BY_ID, null)
      ...
      .then((response) => {
        ...
        setDepartment({ departmentID: ExperienceList[0]?.departmentID, name: response?.data?.getDepartmentByID?.name })
  ...
  const getEditSkills = async () => {
    ...
    if (ExperienceList[0]?.departmentID) {

      const GET_SKILLS_BY_EXP_ID = {
        query: `query MyQuery {
              getSkillsByExpID(expID: ${Number(ExperienceList[0]?.expID)}) {
                name
                skillID
              }
            }`,
...
// console.log(" totalExpData ---------->", totalExpData)
// console.log(" totalExp ---------->", totalExp)

  return (
    <View style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 130 }}>
        {/* ------------------Experiece Confirmation-------------- */}
        <View>
          <Text style={styles.labelText}>Do you have work experience?</Text>
          <View style={styles.qualificationChipView}>
            {
              IsExperienceData?.map((item, i) => (
                <Pressable onPress={() => setIsExperience(item)} key={i} style={[isExperience == item ? styles.chipsSelected : styles.chipsNotSelected]}>
                  <Text style={[isExperience == item ? styles.chipsSelectedText : styles.chipsNotSelectedText]}>{item}</Text>
                </Pressable>
              ))
            }
          </View>
...

        {
          isExperience == 'Yes' ? <View>
            ...
            {jobeRoleErr && <ErrorText title={jobeRoleErr} />}
            ...
            {skillsErr && <ErrorText title={skillsErr} />}
            ...
            {institutionNameErr && <ErrorText title={institutionNameErr} />}
            ...
            <View style={[styles.inputView, { marginVertical: 10 }]}>
              ...
              {joiningDateErr && <ErrorText title={joiningDateErr} />}
            </View>
            ...
            {industryErr && <ErrorText title={industryErr} />}
            {
              ...
              <View style={[styles.inputView, { marginVertical: 10 }]}>
                ...
                {industryOtherErr && <ErrorText title={industryOtherErr} />}
              </View>
            }
            {/* ---------------Department-------------- */}
            ...
            {departmentErr && <ErrorText title={departmentErr} />}
            {
              ...
              <View style={[styles.inputView, { marginVertical: 10 }]}>
                ...
                {departmentOtherErr && <ErrorText title={departmentOtherErr} />}
              </View>
            }
            {/* ---------------Monthly Salary-------------- */}
            <View style={{ marginTop: 10 }}>
              ...
              <View style={styles.monthlySalaryView}>
                ...
                <View style={styles.SalaryinputView}>
                  <TextInput onFocus={() => setActive(true)} onBlur={() => {
                    ...
                    value={monthlySalary.replace(/  +/g, " ")} style={{
                      color: 'black',
                      borderColor: active ? '#395987' : '#B8BCCA50',
                      borderWidth: 1,
                      borderRadius: 10,
              ...
              {monthlySalaryErr && <ErrorText title={monthlySalaryErr} />}
            </View>
      ...
      {
        isExperience && <BottomCoupleButton isLoad={loading} backHandler={backHandler} nextHandler={experienceHandler} />
      }
