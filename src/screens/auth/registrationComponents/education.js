...
import { getEducationList } from '../../../Redux_Mine/Redux_Slices/CompleteProfileTabSlice';
import { showMessage } from 'react-native-flash-message'
import { BackHandler } from 'react-native';


export default function Education({ page, setPage }) {
    let resText = /^[ A-Za-z,.&]*$/;
    ...
    const SaveUpdateEducationDetails = () => {
        ...
        const QUERY_POSTEDUCATION = {
            query: `mutation MyMutation {
                updateEducation(
                courseType: "", 
                eduID: ${Number(EducationList[0]?.eduID) || 0},
                emID: ${specialization?.emID || 0}, 
                universityID: ${Number(university?.umID) || 0},
                yearOfPassing: ${Number(passingYear) || 0},
                otherCourse: "${courseOther ? courseOther : course ? course : ""}", 
    ...
    useEffect(() => {
        ...
        if (EducationList[0]?.qualification) {
            const existData = qualificationMaster?.find((item) => item?.qualification == EducationList[0]?.qualification)
            console.log("exis qual", existData)
            if (existData?.qualification) {
                setQualification({ qualification: EducationList[0]?.qualification })
            }
            else {
                setQualificationOther(EducationList[0]?.qualification)
                setQualification({ qualification: 'Other' })
            }
        }

        if (EducationList[0]?.course) {
            const existData = courseMaster?.find((item) => item?.course == EducationList[0]?.course)
            console.log("exis", existData)
            if (existData?.course) {
                setCourse(EducationList[0]?.course)
            }
            else {
                setCourseOther(EducationList[0]?.course)
                setCourse("Other")
            }
        }

        if (EducationList[0]?.emID) {
            const existData = specializationMaster?.find((item) => item?.specialization == EducationList[0]?.specialization)

            if (existData?.specialization) {
                setSpecialization({ emID: EducationList[0]?.emID, specialization: EducationList[0]?.specialization })
            }
            else {
                setSpecialization({ emID: 0, specialization: EducationList[0]?.specialization })
            }

        }
        if (EducationList[0]?.university) {
            if (EducationList[0]?.isOtherUniversity == false) {
                setUniversity({ name: EducationList[0]?.university, umID: EducationList[0]?.universityID })
            } else {
                setUniversity({ name: EducationList[0]?.university, umID: 0 })
            }
        }
        if (EducationList[0]?.yearOfPassing !== 0) {
            setPassingYear(EducationList[0]?.yearOfPassing)
        }
        if (EducationList[0]?.eduID) {
            getEditSkills()
        }
    }, [EducationList[0]?.yearOfPassing, page]);

    const getEditSkills = async () => {

        const GET_SKILLS_BY_EDU_ID = {
            query: `query MyQuery {
                  getFresherSkillsByEduID(eduID: ${Number(EducationList[0]?.eduID)}) {
                    name
                    skillID
                  }
                }`,
        ...
        <View style={{ flex: 1 }}>
            <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 130 }}>
                ...
                <View>
                    ...
                    {qualificationOtherErr && <ErrorText title={qualificationOtherErr} />}
                </View>

                {
                    qualification?.qualification !== '10/12th' && qualification?.qualification !== 'Below 10th' && qualification?.qualification ? <View>
                        ...
                        <View>
                            ...
                            {courseErr && <ErrorText title={courseErr} />}
                            {
                            ...
                            {courseOtherErr && <ErrorText title={courseOtherErr} />}
                        </View>
                        ...
                        {specializationErr && <ErrorText title={specializationErr} />}
                        {/* ---------------College Name-------------- */}
                        ...
                        {versityErr && <ErrorText title={versityErr} />}
                        {
                        ...
                        {universityOtherErr && <ErrorText title={universityOtherErr} />}
                        ...
                        <View style={styles.inputView}>
                            ...
                            {passingYearErr && <ErrorText title={passingYearErr} />}
                        </View>
            ...
            {
                qualification && <BottomCoupleButton BackBtn={false} isLoad={saveload} backHandler={backHandler} nextHandler={educationHandler} />
            }
